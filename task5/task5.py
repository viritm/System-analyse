...
def get_relation_matrix(ranking: list) -> list:
    ...
    relation_matrix = np.array(
        [[0 for _ in range(ranks_amount)] for _ in range(ranks_amount)])
...
def task(ranking_l: str, ranking_r: str) -> list:
    ...
    conflict_pairs = [[str(conflict[0] + 1), str(conflict[1] + 1)]
                      for conflict in conflicts if conflict[0] < conflict[1]]
